package com.mt.library.imagepicker.activity;
...
import com.mt.library.imagepicker.R;
import com.mt.library.imagepicker.adapter.ImageFoldersAdapter;
import com.mt.library.imagepicker.adapter.ImagePickerAdapter;
import com.mt.library.imagepicker.data.MediaFile;
import com.mt.library.imagepicker.data.MediaFolder;
import com.mt.library.imagepicker.executors.CommonExecutor;
import com.mt.library.imagepicker.listener.MediaLoadCallback;
import com.mt.library.imagepicker.manager.ConfigManager;
import com.mt.library.imagepicker.manager.SelectionManager;
import com.mt.library.imagepicker.task.ImageLoadTask;
import com.mt.library.imagepicker.task.MediaLoadTask;
import com.mt.library.imagepicker.task.VideoLoadTask;
import com.mt.library.imagepicker.utils.DataUtil;
import com.mt.library.imagepicker.utils.MediaFileUtil;
import com.mt.library.imagepicker.utils.PermissionUtil;
import com.mt.library.imagepicker.utils.Utils;
import com.mt.library.imagepicker.view.ImageFolderPopupWindow;
...
public class ImagePickerActivity extends BaseActivity implements ImagePickerAdapter.OnItemClickListener, ImageFoldersAdapter.OnImageFolderChangeListener {
        ...
        if (mediaFile != null) {
            ...
            if (isSingleType) {
                ...
                if (!selectPathList.isEmpty()) {
                    //判断选中集合中第一项是否为视频
                    if (!SelectionManager.isCanAddSelectionPaths(imagePath, selectPathList.get(0).getPath())) {
                        //类型不同
    ...
    private void showCamera() {
        ...
        if (isSingleType) {
            ...
            ArrayList<MediaFile> selectPathList = SelectionManager.getInstance().getSelectPaths();
            if (!selectPathList.isEmpty()) {
                if (MediaFileUtil.isVideoFileType(selectPathList.get(0).getPath())) {
                    //如果存在视频，就不能拍照了
                    Toast.makeText(this, getString(R.string.single_type_choose), Toast.LENGTH_SHORT).show();
                    return;
                }
        ...
        CameraHelper.getInstance(getPackageName()).capturePhoto2Record(this, REQUEST_CODE_CAPTURE, 15000);
        //拍照存放路径
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (resultCode == RESULT_OK) {
            if (requestCode == REQUEST_CODE_CAPTURE) {
                ...
                //添加到选中集合
                MediaFile mediaFile = MediaFile.CREATOR.createFromParcel(Parcel.obtain());
                mediaFile.setPath(path);
                mediaFile.setDuration(realDuration);
